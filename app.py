...
# =====================================================================
# 4. CSS INJECTION (DENGAN TEMA PUTIH & GLOW)
# =====================================================================
def inject_custom_css(bg_base64, logo_base64):
    ...
    #splash-overlay {{ position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 999999; display: flex; justify-content: center; align-items: center; background: #ffffff; animation: overlayFade 2.5s forwards; }}
    ...
    @keyframes headerGlow {{ 
        0%, 100% {{ box-shadow: 0 0 20px rgba(0,77,149,0.6); border-color: rgba(0,77,149,0.9); }} 
        33% {{ box-shadow: 0 0 20px rgba(239,68,68,0.6); border-color: rgba(239,68,68,0.9); }} 
        66% {{ box-shadow: 0 0 20px rgba(130,195,65,0.6); border-color: rgba(130,195,65,0.9); }} 
    }}
...
# =====================================================================
# 5. HEADER & HUD COMPONENT (JS FIXED)
# =====================================================================
def ui_header(logo_base64, pending_count):
    ...
    <div class="header-bar">
        <div style="display:flex; align-items:center; gap:20px;">
            <form action="javascript:window.location.reload()"><button type="submit" class="home-btn" title="Refresh Dashboard"><span class="material-symbols-rounded">home</span></button></form>
            <div>{logo}</div>
        </div>
...
def ui_live_hud_widget():
    ...
    <script>
        // Memperbaiki issue tanda kurung kurawal ganda pada f-string Python
        setInterval(() => {{ 
            const d = new Date(); 
            document.getElementById('clock').innerText = d.toLocaleTimeString('id-ID', {{hour12: false}}); 
            document.getElementById('date').innerText = d.toLocaleDateString('id-ID', {{weekday:'short', day:'numeric', month:'short'}}); 
        }}, 1000);
        ...
        if (navigator.geolocation) {{
            navigator.geolocation.getCurrentPosition(async (pos) => {{
                try {{
                    ...
                    let i = 'partly_cloudy_day', d = 'Berawan'; 
                    if (cw.weathercode === 0) {{ i = 'clear_day'; d = 'Cerah'; }} 
                    else if (cw.weathercode > 50) {{ i = 'rainy'; d = 'Hujan'; }}
                    
                    document.getElementById('w-icon').innerText = i; 
                    document.getElementById('w-desc').innerText = d;
                }} catch(e) {{}}
            }});
        }}
    ...
    for i in range(14):
        ...
        if d_str in df_j.columns:
            ...
            for _, r in day_df.iterrows():
                ...
                s = str(r[d_str]).upper()
                c = "#f97316" if "PD" in s else "#ef4444" if any(x in s for x in ["SAKIT","CUTI","IZIN"]) else "#38bdf8" if n.lower() in subs.get(d_str,[]) else "#22c55e"
                html += f'<div class="scroll-item" style="border-left:3px solid {c}"><b>{n}</b><br><small style="color:{c}">{s}</small></div>'
