...
def ui_live_hud_widget():
    ...
    <script>
        setInterval(()=>{ const d=new Date(); document.getElementById('clock').innerText=d.toLocaleTimeString('id-ID'); document.getElementById('date').innerText=d.toLocaleDateString('id-ID',{weekday:'short',day:'numeric',month:'short'}); },1000);
        ...
        window.addEventListener('deviceorientation', (e)=>{{ if(e.webkitCompassHeading){{ document.getElementById('deg').innerText=Math.round(e.webkitCompassHeading)+'°'; document.getElementById('compass').style.transform=`rotate(${{-e.webkitCompassHeading}}deg)`; }} }});
    </script>
    """, height=90)
